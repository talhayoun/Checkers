const socket = io();
let getRoomNumber = localStorage.Room
...
window.addEventListener("load", ()=>{
    ...
    socket.on("SocketID", (id)=>{
        fetch("http://localhost:3000/updateRoom", {
            method: "POST",
            body: JSON.stringify({username: userName,socketID: id, color:localStorage.Piece}),
            credentials: 'include',
            headers: {
            ...
            let isPlayerOfGame = false
            console.log("Bbbbb")
            console.log(jsonObj)
            for(let i = 0; i< jsonObj.updateRoom.users.length; i++){
                if(jsonObj.updateRoom.users[i].socketID == id){
                    isPlayerOfGame = true;
                }
            }
            console.log(id)
            console.log(isPlayerOfGame)
            if(!isPlayerOfGame){
                location.href = "/"
            }
        })
    ...
    socket.emit("join", getRoomNumber)
    setTimeout(() => {
        fetch(`http://localhost:3000/usersInRoom/${getRoomNumber}`)
        .then((res)=>{
            console.log(res);
            return res.json();
        }).then((jsonObj)=>{
            ...
            if(jsonObj.Users == 2){
                socket.emit("startGame", getRoomNumber)
            }
        }).catch((err)=>{
            console.log(err);
        })
...
// TESTTTT Score
// setTimeout(() => {
//     if(localStorage.Piece == "White"){
//         fetch("http://localhost:3000/updateScore", {
//             method: "POST",
//             body: JSON.stringify({color: "White", room:getRoomNumber}),
//             credentials: 'include',
//             headers: {
//                 "Content-Type": "application/json"
//             }
...
sendMessageButton.addEventListener("click", ()=>{
    ...
    socket.emit("gameMessage", chatMessage.value, localStorage.Username, getRoomNumber);
})
...
socket.on("updateWinner", (winner)=>{
    ...
    getWinnerDiv.innerHTML = winner;
    // getWinnerDiv.id = "";
    getWinnerDiv.className = "wins";
    getWhiteTurn.className = "none";
    getBlackTurn.className = "none";
    getWinnerDiv.style.display = "block"
})

// ---------------------------------------------------------
socket.on("updateBoard", (result)=>{
    updateBoard(result);
    printBoard(result[0])
    gameBoard.children[result[6]].children[0].id = result[6];
    // gameBoard.children[result[6]].id = result[6];
    removeAttributeToPiece(result[6])
})

function updateBoard(res){
    board = res[0];
    totalGameMoves = res[1];
    totalBlackPieces = res[2];
    totalWhitePieces = res[3];
    piecePicked = res[4];
    pickedPiece = res[5];
    setWhoseTurnDiv()
}
function gameData(board, totalGameMoves, totalBlackPieces, totalWhitePieces, piecePicked, pickedPiece, currentPiecePicked){
    let result = [board, totalGameMoves, totalBlackPieces, totalWhitePieces, piecePicked, pickedPiece, currentPiecePicked]
    return result;
}
...
function isItMyTurn(){
    let turn = whoseTurn()
    let myPieceColor = localStorage.Piece;
    if(turn == "whitePiece" && myPieceColor == "White"){
        return true
    }
    if(turn == "blackPiece" && myPieceColor == "Black"){
        return true;
    }
    return false;
}
...
function makeMove(currentPiecePicked){
    ...
    nextRequiredSquare = []
    whiteBlackRequiredSquare = []
    let result = gameData(board, totalGameMoves, totalBlackPieces, totalWhitePieces, piecePicked, pickedPiece, currentPiecePicked)
    socket.emit("newBoard", result, getRoomNumber)
}
...
function isGameOver(){
    ...
    if(isWinner && localStorage.Piece == winnerColor){
        getWinnerDiv.style.display = "block"
        // getWinnerDiv.id = "";
        getWinnerDiv.className = "wins";
        getWhiteTurn.className = "none";
        getBlackTurn.className = "none";
        socket.emit("winner", getRoomNumber ,getWinnerDiv.innerHTML)
        fetch("http://localhost:3000/updateScore", {
            method: "POST",
            body: JSON.stringify({color: winnerColor, room:getRoomNumber}),
            credentials: 'include',
            headers: {
                "Content-Type": "application/json"
            }
